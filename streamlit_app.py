...
# --------------------------------------------------------------------
# Lógica de la Sección 1: Porcentaje de diversidad (MNNAPAM)
# --------------------------------------------------------------------
ETIQUETAS = [
    ("Población total", "PT"), ("Población femenina", "PF"), ("Población masculina", "PM"),
    ("Población de 0 a 14 años", "NNA"), ("Población de 15 a 29 años", "PJ"),
    ("Población de 30 a 59 años", "PA"), ("Población de 60 años y más", "PAM"),
...
def seccion_diversidad():
    ...
    - Población de 0 a 14 años, - Población de 15 a 29 años, - Población de 30 a 59 años
    - Población de 60 años y más, - Población con discapacidad
    """)
    texto = st.text_area(
    ...
    if st.button("Calcular indicadores de diversidad", key="btn_diversidad"):
        ...
        if not texto_limpio:
            ...
            if len(valores) != len(ETIQUETAS):
                st.error("No se detectaron los 8 valores requeridos. Revisa el texto copiado.")
            else:
                PT = valores.get("PT", 0)
                if PT == 0:
                    ...
                    filas = [{
                        "Código": codigo, "Variable": etiqueta, "Valor absoluto": valores.get(codigo, 0),
                        "Porcentaje sobre PT": f"{(valores.get(codigo, 0) / PT) * 100:.2f} %"
                    } for etiqueta, codigo in ETIQUETAS]
    ...
    # --- Bloque de visualización: SOLO se ejecuta si hay resultados ---
    if st.session_state.get("resultado_diversidad") is not None:
        ...
        st.markdown("Este indicador estima la proporción de grupos que pueden tener mayores necesidades de cuidado y accesibilidad en un área.")
        st.latex(r"\text{MNNAPAM} = \frac{ PF + NNA \cdot \frac{PM}{PT} + PAM \cdot \frac{PM}{PT} }{PT} \times 10")
        
        st.subheader("Distribución porcentual")
        st.markdown(f'<div class="stTable tabla-scroll">{df.to_html(index=False)}</div>', unsafe_allow_html=True)


# --------------------------------------------------------------------
# Lógica de la Sección 2: Puntos de accesibilidad y conexión
# --------------------------------------------------------------------
INDICADORES_ACCESO = [
    ("Recubrimiento de la calle", "RDC"), ("Rampa para silla de ruedas", "RSR"), ("Paso peatonal", "PP"),
    ("Banqueta", "BQ"), ("Guarnición", "GN"), ("Ciclovía", "CV"), ("Ciclocarril", "CC"),
    ("Alumbrado público", "AP"), ("Letrero con nombre de la calle", "LNC"), ("Teléfono público", "TP"),
    ("Árboles y palmeras", "ARB"), ("Semáforo para peatón", "SP"), ("Semáforo auditivo", "SA"),
    ("Parada de transporte colectivo", "PTP"), ("Estación para bicicleta", "EBC"),
    ("Alcantarilla de drenaje pluvial", "ADP"), ("Transporte colectivo", "TC"),
    ("Sin restricción del paso a peatones", "SRPP"), ("Sin restricción del paso a automóviles", "SRPA"),
    ("Puesto semifijo", "PS"), ("Puesto ambulante", "PA"),
]
...
def parsear_tabla_accesibilidad(texto: str):
    ...
    for nombre, codigo in INDICADORES_ACCESO:
        ...
        for linea in texto.splitlines():
            ...
            if m:
                valores[codigo] = {
                    "en_todas": int(m.group(1).replace(",", "")), "en_alguna": int(m.group(2).replace(",", "")),
                    "en_ninguna": int(m.group(3).replace(",", "")), "no_especificado": int(m.group(4).replace(",", "")),
...
def calcular_puntajes_acceso(valores_indicadores: dict):
    info_rdc = valores_indicadores.get("RDC", {})
    ...
    puntajes = {codigo: base(info) / TM for codigo, info in valores_indicadores.items()}
    return puntajes, TM
...
def seccion_accesibilidad_conexion():
    ...
    if st.button("Calcular puntos de accesibilidad y conexión", key="btn_acceso"):
        if not (texto_tabla or "").strip():
            ...
            if len(valores_indicadores) != len(INDICADORES_ACCESO):
                st.error("No se detectaron todos los indicadores requeridos. Revisa el texto.")
            else:
                try:
                    ...
                    filas_puntajes = [{"Código": c, "Indicador": n, "Puntaje": f"{puntajes.get(c, 0):.2f}"} for n, c in INDICADORES_ACCESO]
    ...
    if st.session_state.get("resultado_PA") is not None:
        ...
        st.markdown("### ¿Qué evalúan PA y PC?")
        st.markdown("- **PA (Accesibilidad):** Facilidad para caminar y moverse con seguridad.\n- **PC (Conexiones):** Conexión del área con otros puntos de la ciudad.")
        
        st.subheader("Puntaje por indicador")
        st.markdown(f'<div class="stTable tabla-scroll">{df_puntajes.to_html(index=False)}</div>', unsafe_allow_html=True)
